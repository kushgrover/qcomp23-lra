...
pet_ex=partial-exploration/build/distributions/partial-exploration-1.1/bin/partial-exploration
pet_dir=results/pet
if ! [ -e $pet_dir ]
then
  mkdir $pet_dir
...
/usr/bin/time -v -o $pet_dir/rabin.3.time $pet_ex mean-payoff --model=benchmarks/rabin.3.prism --rewards=critical --reward-min=0 --reward-max=1 --pretty-print --global > $pet_dir/rabin.3.log
